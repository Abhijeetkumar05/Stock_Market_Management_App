    ...
    try:
        ...
        for key in required_keys:
            if key not in credentials:
                raise KeyError(f"Missing '{key}' in database_credentials.txt")

        # Connect to MySQL
        connection = mysql.connector.connect(
            host=credentials["host"],  # Should be "127.0.0.1" or "localhost"
            user=credentials["user"],  # Should be "root"
            password=credentials["password"],  # Should match your MySQL root password
            database=credentials["database"],  # Your DB name
            port=int(credentials["port"])  # Port should be an integer (default 3306)
        )
        ...
        print(f"❌ Missing key in credentials file: {e}")
        sys.exit()
